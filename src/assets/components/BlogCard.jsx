...
export function BlogCard({ id, image, category, title, description, author, date }) {
  const formatDate = (dateString) => {
    ...
    // ใช้ Intl.DateTimeFormat เพื่อจัดฟอร์แมตเป็นภาษาอังกฤษ (en-GB จะเรียง วัน เดือน ปี)
  ...
  return (
    <article className="flex flex-col gap-4">
      <Link to={`/post/${id}`} className="overflow-hidden rounded-2xl block transition-opacity hover:opacity-90">
        <img
          src={image}
          alt={title}
      ...
      <div className="flex items-center gap-3 text-sm">
        ...
        {/* เรียกใช้ฟังก์ชัน formatDate ตรงนี้ */}
        <span className="text-[#6b7280]">{formatDate(date)}</span>
      </div>
    </article>
  )
}